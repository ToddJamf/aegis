# Radar — Field Registry
# Aegis stack 2026.06.13
# https://raw.githubusercontent.com/ToddJamf/aegis/main/radar/references/field-registry.md
# Bundled (local). Verified select payloads per tool, Jamf tenant (2026-04-29 / 04-30 / 05-04).
# Copied verbatim from oracle/references/field-registry.md — same verified, scrubbed select set.
...
select = ["Name", "Gsid", "Csm__gr.Name AS CsmName", "Csm",
          ...
          "Customer_Category__gc", "Engagement_Model__gc", "RenewalDate",
...
**Engagement Model** (`Engagement_Model__gc`) is the source of truth for engagement tier — not ARR.
ARR always displays but must never infer engagement cadence/tier.
...
| LOOKUP | — | `Csm`, `PreviousCsm`, `Account_Owner__gc`, `Renewal_Rep__gc`, `Primary_Territory_Owner__gc` |

PICKLIST + `__gr.Name` = `P_5068` (error-handling §A4).

**AE persona field:** `Primary_Territory_Owner__gc` is a LOOKUP — use `Primary_Territory_Owner__gr.Name AS TerritoryOwnerName`. Radar's CSM/AE persona detection scopes the AE book on this field (references/identity.md).
...
*2026.06.13 — Copied into radar/references/ from oracle/references/field-registry.md (already scrubbed + CalVer). Header retitled Oracle → Radar; URL path updated. Added Primary_Territory_Owner__gc to the LOOKUP cheat-sheet + an AE-persona note (Radar scopes the AE book on it). Config/picklist GSIDs retained (records, not people). No customer/employee PII present — confirmed clean.*
